...
$(document).on('ready',function(){
  ...
  // when window loads, browser tries to scroll to element with same id but somehow this breaks header. 
  // waiting until page is fully loaded + skipping few frames avoids strange bahavior (tested IE & chrome)
  // remove with caution :)
  $(window).load(function(){
    setTimeout(function(){
      $((hash != undefined) ? '.fullwidth-tabs li a[data-target="'+ hash +'|top-topic"]' : '.fullwidth-tabs li:first-child a').trigger('click');
    },10);
  });
  ...
  if($('.lesson-sidebar ol').size() > 1){
    
    $('.lesson-sidebar ol').each(function(i,ol){
      
      if($('li.current',this).size() == 0){
        $(this).hide();
        $(this).prev('.bbw').find('h4').addClass('collapsed');
      }
      else {
        $(this).prev('.bbw').find('h4').addClass('collapsed expanded');
      }
      
    });

    $('.lesson-sidebar h4.collapsed').on('click',function(e){
...
$(document).on('ready',function(){
  ...
  $('div.code-snippet').each(function(key) {
    ...
    $this.prepend($(tabs));
    $this.prepend('<div class="copy action tt" data-distance="-40|-30|top"><span></span><div class="tip">Copy code</div></div>');

    if(parseInt($('pre',$this).css('height')) > 260){
      $this.prepend('<div class="expand action tt" data-distance="-40|-30|top"><span></span><div class="tip">Expand view</div></div>');
      $this.prepend('<div class="collapse action tt hide" data-distance="-40|-30|top"><span></span><div class="tip">Collapse view</div></div>');
    }
  });
